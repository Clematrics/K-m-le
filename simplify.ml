...
let rec simplify_ast = function
	FuncCall (s, ast, t) ->
		FuncCall (s, simplify_ast ast, t)
|	Block (b, t) ->
		let b_simple = simplify_instructions b in
		match b_simple with
			[a] -> a
		|	instrs -> Block (simplify_instructions instrs, t)
|	Let (s, l, ast) -> Let (s, l, simplify_ast ast)
|	OpUn (o, ast, t) ->
...
and simplify_instructions expected_type = function
	[] -> []
|	h::t ->
		let ast = simplify_ast h in
		let t = get_ast_type ast in
		if t != UNIT && t = expected_type then
			[ast]
		else
			ast::(simplify_instructions t)
;;